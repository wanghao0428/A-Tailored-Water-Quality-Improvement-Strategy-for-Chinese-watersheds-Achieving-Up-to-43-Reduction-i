...
def get_clean_features_targets(MDVsFile, ratios):
	#从给定的质谱数据文件中读取数据，并根据提供的比率列表提取特征和目标变量。
	'''
	Parameters
	...
	feaCols = list(filter(lambda col: re.search(r'[a-zA-Z]_+[a-zA-Z]', col), ratiosMDVs.columns))
	tarCols = list(filter(lambda col: not re.search(r'[a-zA-Z]_+[a-zA-Z]', col), ratiosMDVs.columns)) if ratios == 'all' else ratios
